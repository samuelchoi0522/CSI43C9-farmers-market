 ...
 * Raised temporarily so the logarithmic “crawl to the target” is easy to see; use ~80 in production.
 */
const TREND_REPORTED_SMOOTH_MS = 500;

/** First paint after mount only: count from 0 → value (ms). Scrubbing uses `TREND_REPORTED_SMOOTH_MS` instead. */
const REPORT_NUM_INTRO_MS = 1000;

/**
 * Multiplier on the logarithmic ease curve. At 1 the curve is subtle; higher = most motion in the first
 * slice of time, then a very long slow finish (good for verifying the easing). Use ~1 for production.
 */
const LOG_EASE_EXAGGERATION = 1;
...
/**
 * Logarithmic ease-out: most motion early, then slows into the target (fast → slow).
 * `t` is linear time in [0, 1].
 */
